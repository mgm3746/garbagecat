...
public class TestParNewPromotionFailedCmsSerialOldPermDataEvent extends TestCase {
	...
	public void testLogLine() {
		...
		Assert.assertEquals("Young begin size not parsed correctly.", (697709 - 664527), event
				.getYoungOccupancyInit());
		Assert.assertEquals("Young end size not parsed correctly.", (317110 - 317110), event
				.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", (1566336 - 1507328),
				event.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 664527, event
				.getOldOccupancyInit());
		Assert.assertEquals("Old end size not parsed correctly.", 317110, event
				.getOldOccupancyEnd());
		Assert.assertEquals("Old allocation size not parsed correctly.", 1507328, event
				.getOldSpace());
		Assert.assertEquals("Perm gen begin size not parsed correctly.", 83780, event
				.getPermOccupancyInit());
		Assert.assertEquals("Perm gen end size not parsed correctly.", 83711, event
				.getPermOccupancyEnd());
	...
	public void testLogLineIncrementalMode() {
		...
		Assert.assertEquals("Young begin size not parsed correctly.", (2022731 - 967034), event
				.getYoungOccupancyInit());
		Assert.assertEquals("Young end size not parsed correctly.", (684015 - 684015), event
				.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", (6191104 - 4886528),
				event.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 967034, event
				.getOldOccupancyInit());
		Assert.assertEquals("Old end size not parsed correctly.", 684015, event
				.getOldOccupancyEnd());
		Assert.assertEquals("Old allocation size not parsed correctly.", 4886528, event
				.getOldSpace());
		Assert.assertEquals("Perm gen begin size not parsed correctly.", 201541, event
				.getPermOccupancyInit());
		Assert.assertEquals("Perm gen end size not parsed correctly.", 201494, event
				.getPermOccupancyEnd());
