...
```
java -jar garbagecat-2.0.x.jar --help
...
Notes:
  ...
  1. Once prepocessing is done once, do not use the -p preprocessing option again, but instead specify the preprocessed file as input (e.g. java -jar /path/to/garbagecat-2.0.x.jar gc.log.pp).
...
```
java -jar garbagecat-2.0.x.jar gc.log
```
...
```
java -jar garbagecat-2.0.x.jar -p gc.log
```
...
```
java -jar garbagecat-2.0.x.jar -t 10 gc.log.pp
```
...
```
java -jar garbagecat-2.0.x.jar -t 10 -s "2009-09-15 10:50:43,343" gc.log.pp
```
