 ...
 * 
 * <p>
 * It looks like this is a result of the young generation guarantee. The young generation fills up
 * to where it exceeds the old generation free space, so a full collection is triggered to free up
 * old space.
 * </p>
 * 
...
public class SerialSerialOldEvent implements BlockingEvent, YoungCollection, OldCollection,
	...
	private static final String REGEX = "^" + JdkRegEx.TIMESTAMP + ": \\[GC " + JdkRegEx.TIMESTAMP
			...
			+ "\\), " + JdkRegEx.DURATION + "\\]" + JdkRegEx.TIMESTAMP + ": \\[Tenured: "
	...
	public SerialSerialOldEvent(String logEntry) {
		...
		Pattern pattern = Pattern.compile(SerialSerialOldEvent.REGEX);
