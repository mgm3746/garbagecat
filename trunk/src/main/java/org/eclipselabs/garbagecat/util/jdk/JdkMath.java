...
public class JdkMath {
	...
	/**
	 * Calculate the throughput between two garbage collection (GC) points. Throughput is the
	 * percent of time not spent doing GC.
	 * 
	 * @param currentDuration
	 *            Current collection time spent doing GC (milliseconds) beginning at
	 *            currentTimestamp.
	 * @param currentTimestamp
	 *            Current collection timestamp (milliseconds after JVM startup).
	 * @param priorDuration
	 *            Prior collection time spent doing GC (milliseconds) beginning at priorTimestamp. 0
	 *            for the first collection.
	 * @param priorTimestamp
	 *            Prior collection timestamp (milliseconds after JVM startup). 0 for the first
	 *            collection.
	 * @return Throughput as a percent. 0 means all time was spent doing GC. 100 means no time was
	 *         spent doing GC.
	 */
	public static int calcThroughput(int currentDuration, long currentTimestamp, int priorDuration,
			long priorTimestamp) {
		long timeTotal = currentTimestamp + new Long(currentDuration).longValue() - priorTimestamp;
		long timeNotGc = timeTotal - new Long(currentDuration).longValue()
				- new Long(priorDuration).longValue();
