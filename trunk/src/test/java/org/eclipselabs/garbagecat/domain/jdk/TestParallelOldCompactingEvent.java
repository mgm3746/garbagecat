...
public class TestParallelOldCompactingEvent extends TestCase {
	...
	public void testLogLine() {
		...
		Assert.assertTrue("Log line not recognized as "
				+ JdkUtil.LogEventType.PARALLEL_OLD_COMPACTING.toString() + ".",
		...
		Assert.assertEquals("Young begin size not parsed correctly.", 1940, event
				.getYoungOccupancyInit());
		Assert
				.assertEquals("Young end size not parsed correctly.", 0, event
						.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", 98560, event
				.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 813929, event
				.getOldOccupancyInit());
		Assert.assertEquals("Old end size not parsed correctly.", 422305, event
				.getOldOccupancyEnd());
		Assert.assertEquals("Old allocation size not parsed correctly.", 815616, event
				.getOldSpace());
		Assert.assertEquals("Perm gen begin size not parsed correctly.", 81960, event
				.getPermOccupancyInit());
		Assert.assertEquals("Perm gen end size not parsed correctly.", 81783, event
				.getPermOccupancyEnd());
		Assert.assertEquals("Perm gen allocation size not parsed correctly.", 164352, event
				.getPermSpace());
	...
	public void testLogLineJdk16() {
		...
		Assert.assertEquals("Young begin size not parsed correctly.", 1788, event
				.getYoungOccupancyInit());
		Assert
				.assertEquals("Young end size not parsed correctly.", 0, event
						.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", 12736, event
				.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 1084, event
				.getOldOccupancyInit());
