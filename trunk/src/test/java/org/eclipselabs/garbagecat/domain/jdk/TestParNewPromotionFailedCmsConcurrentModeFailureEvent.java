...
public class TestParNewPromotionFailedCmsConcurrentModeFailureEvent extends TestCase {
	...
	public void testLogLine() {
		...
		Assert.assertEquals("Young begin size not parsed correctly.", (1292923 - 1048384), event
				.getYoungOccupancyInit());
		Assert.assertEquals("Young end size not parsed correctly.", (1015603 - 1015603), event
				.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", (1441600 - 1179648),
				event.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 1048384, event
				.getOldOccupancyInit());
		Assert.assertEquals("Old end size not parsed correctly.", 1015603, event
				.getOldOccupancyEnd());
	...
	public void testLogLinewithAbortablePreclean() {
		String logLine = "233333.318: [GC 233333.319: [ParNew (promotion failed): "
				+ "673108K->673108K(707840K), 1.5366054 secs]233334.855: [CMS233334.856: "
		...
		Assert.assertEquals("Young begin size not parsed correctly.", (1791073 - 1125100), event
				.getYoungOccupancyInit());
		Assert.assertEquals("Young end size not parsed correctly.", (1156809 - 1156809), event
				.getYoungOccupancyEnd());
		Assert.assertEquals("Young available size not parsed correctly.", (2018560 - 1310720),
				event.getYoungSpace());
		Assert.assertEquals("Old begin size not parsed correctly.", 1125100, event
				.getOldOccupancyInit());
		Assert.assertEquals("Old end size not parsed correctly.", 1156809, event
				.getOldOccupancyEnd());
		Assert.assertEquals("Old allocation size not parsed correctly.", 1310720, event
				.getOldSpace());
