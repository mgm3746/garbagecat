...
public class TestVerboseGcYoungEvent extends TestCase {
	...
	public void testLogLine() {
		...
		Assert.assertTrue("Log line not recognized as "
				+ JdkUtil.LogEventType.VERBOSE_GC_YOUNG.toString() + ".", VerboseGcYoungEvent
				.match(logLine));
		VerboseGcYoungEvent event = new VerboseGcYoungEvent(logLine);
		Assert.assertEquals("Event name incorrect.", JdkUtil.LogEventType.VERBOSE_GC_YOUNG
				.toString(), event.getName());
		Assert.assertEquals("Time stamp not parsed correctly.", 2205570508L, event.getTimestamp());
		Assert.assertEquals("Combined begin size not parsed correctly.", 1726387, event
				.getCombinedOccupancyInit());
		Assert.assertEquals("Combined end size not parsed correctly.", 773247, event
				.getCombinedOccupancyEnd());
		Assert.assertEquals("Combined allocation size not parsed correctly.", 3097984, event
				.getCombinedSpace());
