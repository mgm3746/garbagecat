...
public class TestJdkMath extends TestCase {
	...
	public void testConvertDurationToMillis() {
		...
		Assert.assertEquals("Secs not converted to milliseconds properly.", 22, JdkMath
				.convertSecsToMillis(secs).intValue());
	}
	...
	public void testThroughput() {
		...
		Assert.assertEquals(50, JdkMath.calcThroughput(duration, timestamp, priorDuration,
				priorTimestamp));
	}
	...
	public void testTotalCmsRemarkDuration() {
		...
		Assert.assertEquals("CMS Remark times not added properly.", 170, JdkMath
	...
	public void testConvertDurationMillisToSecs() {
		...
		Assert.assertEquals("Millis not converted to seconds with expected rounding mode.",
				"123.456", JdkMath.convertMillisToSecs(duration).toString());
	}
