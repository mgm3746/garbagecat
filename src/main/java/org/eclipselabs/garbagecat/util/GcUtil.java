...
public class GcUtil {
	 ...
	 * 
	 * @param startDateTime
	 *            The startdatetime <code>String</code> in <code>START_DATE_TIME_REGEX</code>
	 *            format.
	...
	public static final Date parseStartDateTime(String startDateTime) {
		...
		if (matcher.find()) {
			date = getDate(matcher.group(1), matcher.group(2), matcher.group(3), matcher.group(4),
	...
	public static final Date parseDateStamp(String datestamp) {
		...
		if (matcher.find()) {
			date = getDate(matcher.group(2), matcher.group(3), matcher.group(4), matcher.group(5),
	 ...
	 */
	private static final Date getDate(String yyyy, String MM, String dd, String HH, String mm,
			String ss, String SSS) {
		Calendar calendar = Calendar.getInstance();
		if (yyyy == null || MM == null || dd == null || HH == null || mm == null || ss == null
				|| SSS == null) {
	...
	/**
	 * Determine whether the first JVM event timestamp indicates a partial log file or events that
	 * were not in a recognizable format.
	 * 
