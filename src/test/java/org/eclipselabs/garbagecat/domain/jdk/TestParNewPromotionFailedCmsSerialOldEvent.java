...
public class TestParNewPromotionFailedCmsSerialOldEvent extends TestCase {
    ...
    public void testLogLine() {
        String logLine = "1181.943: [GC 1181.943: [ParNew (promotion failed): " + "145542K->142287K(149120K), 0.1316193 secs]1182.075: " + "[CMS: 6656483K->548489K(8218240K), 9.1244297 secs] "
                + "6797120K->548489K(8367360K), 9.2564476 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.LogEventType.PAR_NEW_PROMOTION_FAILED_CMS_SERIAL_OLD.toString() + ".", ParNewPromotionFailedCmsSerialOldEvent.match(logLine));
        ParNewPromotionFailedCmsSerialOldEvent event = new ParNewPromotionFailedCmsSerialOldEvent(logLine);
        Assert.assertEquals("Time stamp not parsed correctly.", 1181943, event.getTimestamp());
        Assert.assertEquals("Young begin size not parsed correctly.", 140637, event.getYoungOccupancyInit());
    ...
    public void testLogLineWithTimesData() {
        String logLine = "1181.943: [GC 1181.943: [ParNew (promotion failed): " + "145542K->142287K(149120K), 0.1316193 secs]1182.075: " + "[CMS: 6656483K->548489K(8218240K), 9.1244297 secs] "
                + "6797120K->548489K(8367360K), 9.2564476 secs] " + "[Times: user=9.52 sys=0.03, real=9.26 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.LogEventType.PAR_NEW_PROMOTION_FAILED_CMS_SERIAL_OLD.toString() + ".", ParNewPromotionFailedCmsSerialOldEvent.match(logLine));
        ParNewPromotionFailedCmsSerialOldEvent event = new ParNewPromotionFailedCmsSerialOldEvent(logLine);
        Assert.assertEquals("Time stamp not parsed correctly.", 1181943, event.getTimestamp());
        Assert.assertEquals("Young begin size not parsed correctly.", 140637, event.getYoungOccupancyInit());
    ...
    public void testLogLineMissingPromotionFailedMessage() {
        String logLine = "3546.690: [GC 3546.691: [ParNew: 532480K->532480K(599040K), 0.0000400 secs]" + "3546.691: [CMS: 887439K->893801K(907264K), 9.6413020 secs] "
                + "1419919K->893801K(1506304K), 9.6419180 secs] " + "[Times: user=9.54 sys=0.10, real=9.65 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.LogEventType.PAR_NEW_PROMOTION_FAILED_CMS_SERIAL_OLD.toString() + ".", ParNewPromotionFailedCmsSerialOldEvent.match(logLine));
        ParNewPromotionFailedCmsSerialOldEvent event = new ParNewPromotionFailedCmsSerialOldEvent(logLine);
    ...
    public void testLogLineTenuredLabel() {
        String logLine = "289985.117: [GC 289985.117: [ParNew (promotion failed): " + "144192K->144192K(144192K), 0.1347360 secs]289985.252: "
                + "[Tenured: 1281600K->978341K(1281600K), 3.6577930 secs] " + "1409528K->978341K(1425792K), 3.7930200 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.LogEventType.PAR_NEW_PROMOTION_FAILED_CMS_SERIAL_OLD.toString() + ".", ParNewPromotionFailedCmsSerialOldEvent.match(logLine));
        ParNewPromotionFailedCmsSerialOldEvent event = new ParNewPromotionFailedCmsSerialOldEvent(logLine);
        Assert.assertEquals("Time stamp not parsed correctly.", 289985117, event.getTimestamp());
        Assert.assertEquals("Young begin size not parsed correctly.", 1409528 - 1281600, event.getYoungOccupancyInit());
        Assert.assertEquals("Young end size not parsed correctly.", 978341 - 978341, event.getYoungOccupancyEnd());
        Assert.assertEquals("Young available size not parsed correctly.", 1425792 - 1281600, event.getYoungSpace());
