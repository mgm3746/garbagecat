...
public class TestParallelCompactingOldEvent {
    ...
    public void testLogLine() {
        ...
        assertEquals("Young begin size not parsed correctly.", 1940, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 98560, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 813929, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 422305, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 815616, event.getOldSpace());
        assertEquals("Perm gen begin size not parsed correctly.", 81960, event.getPermOccupancyInit());
        assertEquals("Perm gen end size not parsed correctly.", 81783, event.getPermOccupancyEnd());
        assertEquals("Perm gen allocation size not parsed correctly.", 164352, event.getPermSpace());
    ...
    public void testLogLineJdk16() {
        ...
        assertEquals("Young begin size not parsed correctly.", 1788, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 12736, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 1084, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 2843, event.getOldOccupancyEnd());
    ...
    public void testLogLineJdk8() {
        ...
        assertEquals("Young begin size not parsed correctly.", 17779, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 1835008, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 16, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 16894, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 4194304, event.getOldSpace());
        assertEquals("Perm gen begin size not parsed correctly.", 19114, event.getPermOccupancyInit());
        assertEquals("Perm gen end size not parsed correctly.", 19114, event.getPermOccupancyEnd());
        assertEquals("Perm gen allocation size not parsed correctly.", 1067008, event.getPermSpace());
    ...
    public void testLogLineLastDitchCollectionTrigger() {
        ...
        assertEquals("Old begin size not parsed correctly.", 773083, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 773083, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 4718592, event.getOldSpace());
        assertEquals("Perm gen begin size not parsed correctly.", 4177368, event.getPermOccupancyInit());
        assertEquals("Perm gen end size not parsed correctly.", 4177368, event.getPermOccupancyEnd());
        assertEquals("Perm gen allocation size not parsed correctly.", 4194304, event.getPermSpace());
    ...
    public void testLogLineErgonomicsTrigger() {
        ...
        assertEquals("Young begin size not parsed correctly.", 105768, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 547840, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 1390311, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 861344, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 1398272, event.getOldSpace());
        assertEquals("Perm gen begin size not parsed correctly.", 136339, event.getPermOccupancyInit());
        assertEquals("Perm gen end size not parsed correctly.", 135256, event.getPermOccupancyEnd());
        assertEquals("Perm gen allocation size not parsed correctly.", 1177600, event.getPermSpace());
    ...
    public void testHeapInspectionInitiatedGcTrigger() {
        ...
        assertEquals("Young begin size not parsed correctly.", 47669, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 1514496, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 2934846, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 851463, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 4718592, event.getOldSpace());
        assertEquals("Metaspace begin size not parsed correctly.", 3959933, event.getPermOccupancyInit());
        assertEquals("Metaspace end size not parsed correctly.", 3959881, event.getPermOccupancyEnd());
        assertEquals("Metaspace allocation size not parsed correctly.", 3977216, event.getPermSpace());
    ...
    public void testAllocationFailureTrigger() {
        ...
        assertEquals("Young begin size not parsed correctly.", 393482, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 393073, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 532992, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 1398224, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 1398199, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 1398272, event.getOldSpace());
        assertEquals("Metaspace begin size not parsed correctly.", 170955, event.getPermOccupancyInit());
        assertEquals("Metaspace end size not parsed correctly.", 170731, event.getPermOccupancyEnd());
        assertEquals("Metaspace allocation size not parsed correctly.", 1220608, event.getPermSpace());
    ...
    public void testHeapDumpInitiatedGcTrigger() {
        ...
        assertEquals("Young begin size not parsed correctly.", 33192, event.getYoungOccupancyInit());
        assertEquals("Young end size not parsed correctly.", 0, event.getYoungOccupancyEnd());
        assertEquals("Young available size not parsed correctly.", 397312, event.getYoungSpace());
        assertEquals("Old begin size not parsed correctly.", 885002, event.getOldOccupancyInit());
        assertEquals("Old end size not parsed correctly.", 812903, event.getOldOccupancyEnd());
        assertEquals("Old allocation size not parsed correctly.", 890368, event.getOldSpace());
        assertEquals("Metaspace begin size not parsed correctly.", 142181, event.getPermOccupancyInit());
        assertEquals("Metaspace end size not parsed correctly.", 141753, event.getPermOccupancyEnd());
        assertEquals("Metaspace allocation size not parsed correctly.", 1185792, event.getPermSpace());
