...
public class TestJdkRegEx extends TestCase {
	...
	public void testTimestampWithCharacter() {
		...
		Assert
				.assertFalse("Timestamps are decimal numbers.", timestamp
						.matches(JdkRegEx.TIMESTAMP));
	}

	public void testTimestampWithFewerDecimalPlaces() {
		String timestamp = "1.12";
		Assert.assertFalse("Timestamps have 3 decimal places.", timestamp
				.matches(JdkRegEx.TIMESTAMP));
	}
	...
	public void testTimestampLessThanOne() {
		...
		Assert.assertTrue("Timestamps less than one do not have a leading zero.", timestamp
				.matches(JdkRegEx.TIMESTAMP));
	}
	...
	public void testDuration7LessThanOne() {
		...
		Assert.assertFalse("Durations less than one have a leading zero.", duration
				.matches(JdkRegEx.DURATION));
	}
	...
	public void testDuration7Valid() {
		...
		Assert.assertTrue("'0.0225213 secs' is a valid duration.", duration
				.matches(JdkRegEx.DURATION));
	}
	...
	public void testParallelScavengeValid() {
		...
		Assert.assertTrue("'19810.091: [GC [PSYoungGen: 27808K->632K(28032K)] "
				+ "160183K->133159K(585088K), 0.0225213 secs]' "
				+ "is a valid parallel scavenge log entry.", ParallelScavengeEvent.match(logLine));
	}
	...
	public void testTimesBlock4Digits() {
		...
		Assert.assertTrue("'" + timesBlock + "' " + "is a valid times block.", timesBlock
				.matches(JdkRegEx.TIMES_BLOCK));
	}
}
