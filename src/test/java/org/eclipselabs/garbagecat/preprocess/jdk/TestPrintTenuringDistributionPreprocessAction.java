...
public class TestPrintTenuringDistributionPreprocessAction extends TestCase {
    ...
    public void testDefNewLine() {
        ...
        Assert.assertTrue("Log line not recognized as " + JdkUtil.PreprocessActionType.PRINT_TENURING_DISTRIBUTION.toString() + ".", PrintTenuringDistributionPreprocessAction.match(logLine, priorLogLine));
    }
    ...
    public void testLineBeforeConcurrentModeFailure() {
        String priorLogLine = null;
        String logLine = ": 917504K->917504K(917504K), 5.5887120 secs]877375.047: [CMS877378.691: " + "[CMS-concurrent-mark: 5.714/11.380 secs] [Times: user=14.72 sys=4.81, real=11.38 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.PreprocessActionType.PRINT_TENURING_DISTRIBUTION.toString() + ".", PrintTenuringDistributionPreprocessAction.match(logLine, priorLogLine));
    }

    public void testAbortPrecleanLineBeforeConcurrentModeFailure() {
        String priorLogLine = null;
        String logLine = ": 910978K->910978K(917504K), 1.7096800 secs]891199.155: [CMS CMS: abort preclean " + "due to time 891199.474: [CMS-concurrent-abortable-preclean: 4.227/6.007 secs] "
    ...
    public void testParallelScavengeLastLine() {
        ...
        String logLine = " [PSYoungGen: 970752K->104301K(1456128K)] 970752K->104301K(3708928K), 0.1992940 secs] [Times: user=0.68 sys=0.05, real=0.20 secs]";
        Assert.assertTrue("Log line not recognized as " + JdkUtil.PreprocessActionType.PRINT_TENURING_DISTRIBUTION.toString() + ".", PrintTenuringDistributionPreprocessAction.match(logLine, priorLogLine));
    }
}
